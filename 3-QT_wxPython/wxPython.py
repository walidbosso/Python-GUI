...
class EmployeeUI(wx.Frame):
    def __init__(self):
        super(EmployeeUI, self).__init__(None, title="EmployeeUI Window", size=(600, 400))
        self.panel = wx.Panel(self)
    ...
    def edit_selected_employee(self, event):
        ...
        if selected_rows:
            ...
            cursor.execute("UPDATE employee SET name=?, salary=?, phone=? WHERE employee_id=?", (name, salary, phone, employee_id))
            sqliteConnection.commit()
...
class ProductUI(wx.Frame):
    def __init__(self):
        super(ProductUI, self).__init__(None, title="ProductUI Window", size=(600, 400))
        self.panel = wx.Panel(self)
    ...
    def edit_selected_product(self, event):
        ...
        if selected_rows:
            ...
            cursor.execute("UPDATE product SET name=?, description=?, price=?, category=? WHERE product_id=?", (name, description, price, category, product_id))
            sqliteConnection.commit()
...
class PurchaseUI(wx.Frame):
    def __init__(self):
        super(PurchaseUI, self).__init__(None, title="PurchaseUI Window", size=(600, 400))
        self.panel = wx.Panel(self)
    ...
    def edit_selected_purchase(self, event):
        ...
        if selected_rows:
            ...
            cursor.execute("UPDATE purchase SET customer_id=?, product_id=?, employee_id=?, price=?, quantity=?, purchase_date=? WHERE purchase_id=?", (customer_id, product_id, employee_id, price, quantity, date, purchase_id))
            sqliteConnection.commit()
